...
#define TOKEN_TICK_USEC 100000
#define TOKEN_TICKS_PER_SEC (1000000 / TOKEN_TICK_USEC)

struct token_entyr
{
    uint32_t brust_token;
    uint32_t speed_token;
    uint32_t my_token;
    uint32_t token_remainder;
    int pos_token;
...
static void refill_one_bucket(struct token_entyr *bucket)
{
    uint32_t pending_tokens;
    uint32_t add_tokens;

    pending_tokens = bucket->speed_token + bucket->token_remainder;
    add_tokens = pending_tokens / TOKEN_TICKS_PER_SEC;
    bucket->token_remainder = pending_tokens % TOKEN_TICKS_PER_SEC;

    if (add_tokens == 0)
        return;

    bucket->my_token += add_tokens;
    if (bucket->my_token > bucket->brust_token)
        bucket->my_token = bucket->brust_token;
}
...
static void buff_init(void)
{
    ...
    for (i = 0; i < TOKEN_MAXNUM; i++)
    {
        ...
        token_buff[i].token_remainder = 0;
...
struct token_entyr *token_init(uint32_t brust_token, uint32_t speed_token)
{
    ...
    token_buff[pos].my_token = brust_token;
    token_buff[pos].token_remainder = 0;
...
{
    if (ptr == NULL || need_token == 0 || need_token > ptr->brust_token)
        return -1;
